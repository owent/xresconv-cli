            ...
            if tag_name == "work_dir":
                ...
                if tag_rule and tag_rule.strip():
                    output_rule["tags"] = set(
                        filter(
                            lambda x: x, xconv_split_by_spaces.split(tag_rule.strip())
                        )
                    )
                ...
                if class_rule and class_rule.strip():
                    output_rule["classes"] = set(
                        filter(
                            lambda x: x, xconv_split_by_spaces.split(class_rule.strip())
                        )
                    )
                ...
                if "name" in global_option.attrib:
                    ...
                    if scheme_key in xconv_options["default_scheme"]:
                        xconv_options["default_scheme"][scheme_key].append(trip_value)
                    else:
...
conv_start_msg = ("[NOTICE] start to run conv cmds on dir: {0}" + os.linesep).format(
    os.getcwd()
)
    ...
    for item_info in lis:
        ...
        if "class" in item.attrib:
            conv_item_obj["classes"] = set(
                filter(lambda x: x, xconv_split_by_spaces.split(item.attrib["class"]))
            )
        ...
        for key in xconv_options["default_scheme"]:
            if key not in conv_item_obj["scheme_data"]:
                conv_item_obj["scheme_data"][key] = xconv_options["default_scheme"][key]
...
def worker_func(idx):
    ...
    pexec = None
    if not options.test:
        pexec = Popen(java_options, stdin=PIPE, stdout=PIPE, stderr=PIPE, shell=False)
